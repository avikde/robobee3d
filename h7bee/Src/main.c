...
/* USER CODE BEGIN PV */
// // adc_results_mem symbol is hardcoded in linker script so don't change it without updating it there
// #define RAM_D2 __attribute__((section(".d2ram")))
// volatile RAM_D2 uint32_t adc_results_mem[4];
/* USER CODE END PV */
...
int main(void)
{
  ...
  // Start all the things that need to be started
  ...
  // for (int i = 0; i < 4; ++i)
  //   adc_results_mem[i] = 0;
  // HAL_ADC_Start_DMA(&hadc1, (uint32_t *)adc_results_mem, 4); // returns HAL_OK
  // HAL_ADC_Start(&hadc1);
  HAL_ADCEx_InjectedStart(&hadc1);

  // Flap task timebase (timer interrupt)
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  PeriphClkInitStruct.PeriphClockSelection = RCC_PERIPHCLK_USART2|RCC_PERIPHCLK_USART1
                              |RCC_PERIPHCLK_UART5|RCC_PERIPHCLK_SPI2
                              |RCC_PERIPHCLK_ADC;
...
static void MX_USART2_UART_Init(void)
{
  ...
  huart2.AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_DATAINVERT_INIT;
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOD, LED1_Pin|LED2_Pin|SSIMU_Pin, GPIO_PIN_SET);

  /*Configure GPIO pins : LED1_Pin LED2_Pin SSIMU_Pin */
  GPIO_InitStruct.Pin = LED1_Pin|LED2_Pin|SSIMU_Pin;
