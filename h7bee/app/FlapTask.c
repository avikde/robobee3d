...
static void voltageControl(float vdes, float vact, TIM_HandleTypeDef *htim)
{
	...
	// Try simple P control with saturation
	const float kMagDuty = 5, kP = 0.03; // 0.02 for 0.1uF
	float mag = constrain(kP * (vact - vdes), -kMagDuty, kMagDuty);
	if (vact > vdes)
	{
		dcl = mag;
		dch = 0;
	...
	{
		dch = -mag; // based on looking at sine output
		dcl = 0;
	}
