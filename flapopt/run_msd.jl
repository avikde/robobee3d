...
function debugComponentsPlot(traj, param; refScale=1.0)
	...
	for k=1:N
		...
		yn = yo(k+1)
		Ht(Hi) = cu.Hτ(Hi, y[1])
		inertialo[k] = [Ht(Hio(y,yn))  H0  H0] ⋅ pt0
		inertiala[k] = [Ht(Hia(y,yn))  H0  H0] ⋅ pt0
		stiffo[k] = [H0  H0  Ht(Hstiffo(y))] ⋅ pt0
		stiffa[k] = [H0  H0  Ht(Hstiffa(y))] ⋅ pt0
		damp[k] = [H0  Ht(Hdamp(y))  H0] ⋅ pt0
	end
